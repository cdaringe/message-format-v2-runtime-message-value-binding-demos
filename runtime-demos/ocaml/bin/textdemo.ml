...
let create_tags (buffer:GText.buffer) =
  let _ = buffer#create_tag ~name:"heading" [`WEIGHT `BOLD; `SIZE (15*Pango.scale)] in
  let _ = buffer#create_tag ~name:"italic" [`STYLE `ITALIC] in
  let _ = buffer#create_tag ~name:"bold" [`WEIGHT `BOLD] in
  let _ = buffer#create_tag ~name:"big" [`SIZE 20] in
  let _ = buffer#create_tag ~name:"xx-small" [`SCALE `XX_SMALL] in
  let _ = buffer#create_tag ~name:"x-large" [`SCALE `X_LARGE] in
  let _ = buffer#create_tag ~name:"monospace" [`FAMILY "monospace"] in
  let _ = buffer#create_tag ~name:"blue_foreground" [`FOREGROUND "blue"] in
  let _ = buffer#create_tag ~name:"red_background" [`BACKGROUND "red"] in
  let _ = buffer#create_tag ~name:"big_gap_before_line" [`PIXELS_ABOVE_LINES 30] in
  let _ = buffer#create_tag ~name:"big_gap_after_line" [`PIXELS_BELOW_LINES 30] in
  let _ = buffer#create_tag ~name:"double_spaced_line" [`PIXELS_INSIDE_WRAP 10] in
  let _ = buffer#create_tag ~name:"not_editable" [`EDITABLE false] in
  let _ = buffer#create_tag ~name:"word_wrap" [`WRAP_MODE `WORD] in
  let _ = buffer#create_tag ~name:"char_wrap" [`WRAP_MODE `CHAR] in
  let _ = buffer#create_tag ~name:"no_wrap" [`WRAP_MODE `NONE] in
  let _ = buffer#create_tag ~name:"center" [`JUSTIFICATION `CENTER] in
  let _ = buffer#create_tag ~name:"right_justify" [`JUSTIFICATION `RIGHT] in
  let _ = buffer#create_tag ~name:"wide_margins" [`LEFT_MARGIN  50; `RIGHT_MARGIN 50] in
  let _ = buffer#create_tag ~name:"strikethrough" [`STRIKETHROUGH true] in
  let _ = buffer#create_tag ~name:"underline" [`UNDERLINE `SINGLE] in
  let _ = buffer#create_tag ~name:"double_underline" [`UNDERLINE `DOUBLE] in
  let _ = buffer#create_tag ~name:"superscript" [`RISE (10*Pango.scale); `SIZE (8*Pango.scale)] in
  let _ = buffer#create_tag ~name:"subscript" [`RISE (-10*Pango.scale); `SIZE (8*Pango.scale)] in
  ()
...
let insert_text (buffer:GText.buffer) =
  ...
  buffer#insert ~iter ~tag_names:["wide_margins"]
   ...
   ignore @@ buffer#create_child_anchor iter;
   buffer#insert ~iter
       " and a menu : ";
       ignore @@  buffer#create_child_anchor iter;
   buffer#insert ~iter
...
let rec find_anchor (iter : GText.iter) =
  ...
  match iter#nocopy#forward_char |> ignore ; iter#contents with
    `CHILD _ -> true
  | _ -> find_anchor iter
...
let easter_egg_callback =
  ...
  fun () -> match !window with Some w -> w#present ()
	...
	w'#connect#destroy ~callback:(fun () -> window:=None) |> ignore;
	window := Some w';
...
let attach_widgets (text_view:GText.view) =
  ...
  while find_anchor iter do
	...
	b#connect#clicked ~callback:easter_egg_callback |> ignore;
	b#coerce
      | 1 -> let menu = GMenu.menu () in
	let widget = GMenu.menu_item () in
...
let main () =
  let window = GWindow.window ~width:450 ~height:450 ~title:"TextView"
      ~border_width:0 () in
  window#connect#destroy ~callback:(fun _ -> exit 0) |> ignore;
